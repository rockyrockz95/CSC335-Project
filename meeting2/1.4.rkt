...
;For identifiers,it also does not create a closure that capture environment, it just refer to it's environment passed with the function meaning. In a closure form, its environment will be the the environment captured by closure, which ensure the identifier will be resolved using its lexical definition.
